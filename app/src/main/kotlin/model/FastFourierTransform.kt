...
/**
 * ***********************************************************************************************
 * Fast Fourier Transform -- Kotlin Version This version implements Cooley-Tukey algorithm for composite-length
 * sequences.
 *
 ...
 * $ kotlinc anyfft.kt
 *
 * It will generate a file named 'FftKt.class' in the same directory. It can be run by issuing the command:
 *
 * $ kotlin AnyfftKt
 * ************************************************************************************************
 */
...
/**
 * ***********************************************************************************************
 * Auxiliary Function: complexShow Pretty printing of an array of complex numbers, used to inspect results.
 *
 * Parameters: x A vector of complex numbers, according to the definition above;
 * ************************************************************************************************
 */
fun complexShow(x: Array<Complex>) {
    for (i in 0..x.size - 1) println("( " + x[i].r + ", " + x[i].i + ")")
}

/**
 * ***********************************************************************************************
 * Auxiliary Function: timeIt Measure execution time through repeated calls to a (Fast) Fourier Transform function.
 *
 * Parameters: f Function to be called, with the given prototype. The first complex vector is the input vector, the
 * second complex vector is the result of the computation, and the integer is the number of elements in the vector; size
 * Number of elements in the vector on which the transform will be applied; repeat Number of times the function will be
 * called.
 *
 * Returns: The average execution time for that function with a vector of the given size.
 * ************************************************************************************************
 */
...
/**
 * ***********************************************************************************************
 * Function: directFT Discrete Fourier Transform directly from the definition, an algorithm that has O(N^2) complexity.
 *
 * Parameters: x The vector of which the DFT will be computed. Given the nature of the implementation, there is no
 * restriction on the size of the vector, although it will almost always be called with a power of two size to give a
 * fair comparison;
 *
 * Returns: A complex-number vector of the same size, with the coefficients of the DFT.
 * ************************************************************************************************
 */
...
/**
 * ***********************************************************************************************
 * Function: factor Smallest prime factor of a given number. If the argument is prime itself, then it is the return
 * value.
 *
 * Parameters: n Number to be inspected.
 *
 * Returns: The smallest prime factor, or the number itself if it is already a prime.
 * ************************************************************************************************
 */
...
/**
 * ***********************************************************************************************
 * Function: recursiveFFT Fast Fourier Transform using a recursive decimation in time algorithm. This has smaller
 * complexity than the direct FT, though the exact value is difficult to compute.
 *
 * Parameters: x The vector of which the FFT will be computed. Its length must be a composite number, or else the
